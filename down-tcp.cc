...
int main (int argc, char *argv[]){
		...
		while(t<5){
			...
			//for(int k = nNode-1 ; k >= 0 ; k--){
  			...
  			sources[k] = new BulkSendHelper("ns3::TcpSocketFactory",InetSocketAddress(i.GetAddress(3+k), port));
			(*sources[k]).SetAttribute ("MaxBytes", UintegerValue (0));
			sourceApps[k] = (*sources[k]).Install (nodes.Get (0));
			...
			for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
			{
				...
				if(t.sourceAddress=="10.1.1.1"&&(t.destinationAddress=="10.1.2.2"||t.destinationAddress=="10.1.2.3"||t.destinationAddress=="10.1.2.4"||t.destinationAddress=="10.1.2.5")){
